...
(function () {
  ...
  var FLIP_MS   = 1050; // 電腦／平板：要跟 style.css 裡 .leaf 的 transition 時間一致
  var FLIP_MS_M = 620;  // 手機（不分直橫）：翻頁短一點才跟手（要跟 style.css 的 body.thin .leaf 一致）
  /* 用 thin 而不是 mobile：手機橫放是雙頁跨頁（mobile=false），但硬體還是手機。 */
  function flipMs() { return thin ? FLIP_MS_M : FLIP_MS; }
